...
(function dartProgram() {
  ...
  var A = {JS_CONST: function JS_CONST() {
    ...
    DisplaySystem$(container, filter) {
      var t1 = J.JSArray_JSArray$growable(0, type$.UserProfile);
      t1 = new A.DisplaySystem(t1, container, filter);
      t1.DisplaySystem$2(container, filter);
      return t1;
    },
    main() {
      var t1 = document,
        result = t1.getElementById("result"),
        filter = t1.getElementById("filter");
      if (type$.Element._is(result) && type$.TextInputElement._is(filter))
        A.DisplaySystem$(result, filter);
    },
    AllowAll: function AllowAll() {
    },
    UserProfile: function UserProfile(t0, t1, t2, t3, t4) {
      var _ = this;
      _.name = t0;
      _.surname = t1;
      _.imgsrc = t2;
      _.city = t3;
      _.country = t4;
      _.__UserProfile_element_A = $;
    },
    DisplaySystem: function DisplaySystem(t0, t1, t2) {
      this.profiles = t0;
      this.container = t1;
      this.filter = t2;
    },
    DisplaySystem_closure: function DisplaySystem_closure(t0) {
      this.$this = t0;
    },
  ...
  A.InputElement.prototype = {$isTextInputElement: 1};
  ...
  A.UserProfile.prototype = {
    meetsCriteria$1(searchTerm) {
      var t1, _i, string, _this = this;
      searchTerm = searchTerm.toLowerCase();
      for (t1 = [_this.name, _this.surname, _this.imgsrc, _this.city, _this.country], _i = 0; _i < 5; ++_i) {
        string = t1[_i].toLowerCase();
        if (A.stringContainsUnchecked(string, searchTerm.toLowerCase(), 0))
          return true;
      }
      return false;
    }
  };
  A.DisplaySystem.prototype = {
    DisplaySystem$2(container, filter) {
      this.getData$0();
      B.InputElement_methods.addEventListener$2(this.filter, "input", new A.DisplaySystem_closure(this));
    },
    getData$0() {
      var $async$goto = 0,
        $async$completer = A._makeAsyncAwaitCompleter(type$.void),
        $async$self = this, t2, user, t3, $name, surname, imgsrc, city, country, profile, li, results, t1, $async$temp1, $async$temp2, $async$temp3;
        ...
        while (true)
          switch ($async$goto) {
            case 0:
              ...
              $async$goto = 2;
              return A._asyncAwait(A.HttpRequest_getString("https://randomuser.me/api?results=50"), $async$getData$0);
            case 2:
              // returning from await.
              results = $async$temp1._as($async$temp2.$index$asx($async$temp3.decode$1(0, $async$result), "results"));
              ...
              for (t2 = J.get$iterator$ax(results); t2.moveNext$0();) {
                ...
                $name = A._asString(J.$index$asx(t3.$index(user, "name"), "first"));
                surname = A._asString(J.$index$asx(t3.$index(user, "name"), "last"));
                imgsrc = A._asString(J.$index$asx(t3.$index(user, "picture"), "large"));
                city = A._asString(J.$index$asx(t3.$index(user, "location"), "city"));
                country = A._asString(J.$index$asx(t3.$index(user, "location"), "country"));
                profile = new A.UserProfile($name, surname, imgsrc, city, country);
                li = document.createElement("li");
                t3 = "" + ('<img src="' + imgsrc + '" alt="' + $name + '">') + '<div class="user-info">' + ("<h4>" + $name + " " + surname + "</h4>") + ("<p>" + city + ", " + country + "</p>") + "</div>";
                J.setInnerHtml$2$validator$x(li, t3.charCodeAt(0) == 0 ? t3 : t3, new A.AllowAll());
                profile.__UserProfile_element_A = li;
                B.JSArray_methods.add$1(t1, profile);
              }
              $async$self.createElements$0();
              // implicit return
    ...
    createElements$0() {
      var t3, t4, _i, profile, t5, t6,
        t1 = this.container,
        t2 = J.getInterceptor$x(t1);
      t2.get$children(t1).clear$0(0);
      for (t3 = this.profiles, t4 = t3.length, _i = 0; _i < t3.length; t3.length === t4 || (0, A.throwConcurrentModificationError)(t3), ++_i) {
        profile = t3[_i];
        t5 = t2.get$children(t1);
        t6 = profile.__UserProfile_element_A;
        t6 === $ && A.throwLateFieldNI("element");
        t5.add$1(0, t6);
      }
    },
    filterElements$1(searchTerm) {
      var t1, t2, _i, profile, t3, _s4_ = "hide";
      for (t1 = this.profiles, t2 = t1.length, _i = 0; _i < t1.length; t1.length === t2 || (0, A.throwConcurrentModificationError)(t1), ++_i) {
        profile = t1[_i];
        t3 = profile.__UserProfile_element_A;
        t3 === $ && A.throwLateFieldNI("element");
        if (!J.get$classes$x(t3).contains$1(0, _s4_))
          J.get$classes$x(profile.__UserProfile_element_A).add$1(0, _s4_);
        if (profile.meetsCriteria$1(searchTerm))
          J.get$classes$x(profile.__UserProfile_element_A).remove$1(0, _s4_);
      }
    }
  };
  A.DisplaySystem_closure.prototype = {
    call$1($event) {
      var t1, t2;
