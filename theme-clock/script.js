...
(function dartProgram() {
  ...
  var A = {JS_CONST: function JS_CONST() {
    ...
    ThemeClock$() {
      var t1 = J.JSArray_JSArray$growable(0, type$.TimeElement),
        t2 = type$.dynamic,
        t3 = type$.String;
      t3 = new A.ThemeClock(t1, new A.TimeKeeper(A.List_List$from(A.LinkedHashSet_LinkedHashSet$_literal(["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"], t2), t3), A.List_List$from(A.LinkedHashSet_LinkedHashSet$_literal(["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"], t2), t3)));
      t3.ThemeClock$0();
      return t3;
    },
    ThemeToggle$(element) {
      var t1 = new A.ThemeToggle(element);
      t1.ThemeToggle$1(element);
      return t1;
    },
    main() {
      var dateEl, timeEl,
        themeClock = A.ThemeClock$(),
        t1 = document,
        hourEl = t1.querySelector(".hour"),
        minuteEl = t1.querySelector(".minute"),
        secondEl = t1.querySelector(".second"),
        t2 = type$.Element;
      if (t2._is(hourEl) && t2._is(minuteEl) && t2._is(secondEl))
        themeClock.includeElement$1(new A.ClockDisplay(hourEl, minuteEl, secondEl));
      dateEl = t1.querySelector(".date");
      timeEl = t1.querySelector(".time");
      if (t2._is(dateEl) && t2._is(timeEl))
        themeClock.includeElement$1(new A.DateTimeDisplay(dateEl, timeEl));
    },
    AllowAll: function AllowAll() {
    },
    ThemeClock: function ThemeClock(t0, t1) {
      this.elements = t0;
      this.keeper = t1;
    },
    ThemeClock_closure: function ThemeClock_closure(t0) {
      this.$this = t0;
    },
    ThemeToggle: function ThemeToggle(t0) {
      this.element = t0;
    },
    ThemeToggle_closure: function ThemeToggle_closure(t0) {
      this.$this = t0;
    },
    TimeElement0: function TimeElement0() {
    },
    TimeKeeper: function TimeKeeper(t0, t1) {
      var _ = this;
      _.month = "";
      _.day = 0;
      _.weekday = "";
      _.seconds = _.minutes = _.hours = 0;
      _.months = t0;
      _.weekdays = t1;
    },
    DateTimeDisplay: function DateTimeDisplay(t0, t1) {
      this.dateEl = t0;
      this.timeEl = t1;
    },
    ClockDisplay: function ClockDisplay(t0, t1, t2) {
      this.hourEl = t0;
      this.minuteEl = t1;
      this.secondEl = t2;
    },
  ...
  A.ThemeClock.prototype = {
    ThemeClock$0() {
      var toggle = document.querySelector(".toggle");
      if (type$.ButtonElement._is(toggle))
        A.ThemeToggle$(toggle);
      A.Timer_Timer$periodic(new A.Duration(1000000), new A.ThemeClock_closure(this));
    },
    includeElement$1(element) {
      var t1 = this.elements;
      if (!B.JSArray_methods.contains$1(t1, element))
    ...
    updateAll$0() {
      var _i,
        ...
        t2 = t1.months,
        t3 = A.Primitives_getMonth(now) - 1;
      if (!(t3 >= 0 && t3 < t2.length))
        return A.ioore(t2, t3);
      t1.set$month(t2[t3]);
      t1.day = A.Primitives_getDay(now);
      t3 = t1.weekdays;
      t2 = A.Primitives_getWeekday(now) - 1;
      if (!(t2 >= 0 && t2 < t3.length))
        return A.ioore(t3, t2);
      t1.set$weekday(t3[t2]);
      t1.hours = A.Primitives_getHours(now);
      t1.minutes = A.Primitives_getMinutes(now);
      t1.seconds = A.Primitives_getSeconds(now);
      for (t2 = this.elements, t3 = t2.length, _i = 0; _i < t2.length; t2.length === t3 || (0, A.throwConcurrentModificationError)(t2), ++_i)
        t2[_i].update$1(t1);
    }
  };
  A.ThemeClock_closure.prototype = {
    call$1(timer) {
      this.$this.updateAll$0();
    },
    $signature: 16
  };
  A.ThemeToggle.prototype = {
    ThemeToggle$1(element) {
      B.ButtonElement_methods._addEventListener$3(this.element, "click", type$.nullable_dynamic_Function_Event._as(new A.ThemeToggle_closure(this)), null);
    }
  };
  A.ThemeToggle_closure.prototype = {
    call$1($event) {
      var html, t1, t2;
      type$.Event._as($event);
      html = document.querySelector("html");
      if (type$.Element._is(html)) {
        t1 = J.getInterceptor$x(html);
        t1.get$classes(html).toggle$1(0, "dark");
        t2 = this.$this.element;
        if (t1.get$classes(html).contains$1(0, "dark"))
          t2.innerText = "Light mode";
        else
          t2.innerText = "Dark mode";
      }
    },
    $signature: 17
  };
  A.TimeElement0.prototype = {};
  A.TimeKeeper.prototype = {
    getClockHour$0() {
      var t1 = this.hours;
      return t1 > 12 ? B.JSInt_methods.$mod(t1, 12) : t1;
    },
    getClockMins$0() {
      var t1 = this.minutes;
      if (t1 < 10)
    ...
    set$month(month) {
      this.month = A._asString(month);
    },
    set$weekday(weekday) {
      this.weekday = A._asString(weekday);
    }
  };
  A.DateTimeDisplay.prototype = {
    update$1(time) {
      var t1, t2, t3;
      J.setInnerHtml$2$validator$x(this.dateEl, time.weekday + ", " + time.month + ' <span class="circle">' + time.day + "</span>", new A.AllowAll());
      t1 = time.getClockHour$0();
      t2 = time.getClockMins$0();
      t3 = time.hours < 12 ? "AM" : "PM";
      J.setInnerHtml$2$validator$x(this.timeEl, "" + t1 + ":" + t2 + " " + t3, new A.AllowAll());
    }
  };
  A.ClockDisplay.prototype = {
    update$1(time) {
      var t2, _this = this,
        _s30_ = "translate(-50%, -100%) rotate(",
        _s9_ = "transform",
        t1 = _this.hourEl.style;
      t1.toString;
      t2 = _this.scale$5(0, time.getClockHour$0(), 0, 12, 0, 360);
      B.CssStyleDeclaration_methods._setPropertyHelper$3(t1, B.CssStyleDeclaration_methods._browserPropertyName$1(t1, _s9_), _s30_ + A.S(t2) + "deg)", "");
      t2 = _this.minuteEl.style;
      t2.toString;
      t1 = _this.scale$5(0, time.minutes, 0, 60, 0, 360);
      B.CssStyleDeclaration_methods._setPropertyHelper$3(t2, B.CssStyleDeclaration_methods._browserPropertyName$1(t2, _s9_), _s30_ + A.S(t1) + "deg)", "");
      t1 = _this.secondEl.style;
      t1.toString;
      t2 = _this.scale$5(0, time.seconds, 0, 60, 0, 360);
      B.CssStyleDeclaration_methods._setPropertyHelper$3(t1, B.CssStyleDeclaration_methods._browserPropertyName$1(t1, _s9_), _s30_ + A.S(t2) + "deg)", "");
    },
    scale$5(_, num, in_min, in_max, out_min, out_max) {
  ...
  (function inheritance() {
    ...
    _inheritMany(A.Object, [A.JS_CONST, J.Interceptor, J.ArrayIterator, A.Error, A.Iterable, A.ListIterator, A.WhereIterator, A.TypeErrorDecoder, A.NullThrownFromJavaScriptException, A._StackTrace, A.Closure, A.MapBase, A.LinkedHashMapCell, A.LinkedHashMapKeyIterator, A.JSSyntaxRegExp, A.Rti, A._FunctionParameters, A._Type, A._TimerImpl, A._AsyncCallbackEntry, A._Zone, A.SetBase, A._LinkedHashSetCell, A._LinkedHashSetIterator, A.ListBase, A.DateTime, A.Duration, A.OutOfMemoryError, A.StackOverflowError, A._Exception, A.FormatException, A.Null, A.StringBuffer, A.CssStyleDeclarationBase, A._Html5NodeValidator, A.ImmutableListMixin, A.NodeValidatorBuilder, A._SimpleNodeValidator, A.FixedSizeListIterator, A._SameOriginUriPolicy, A._ValidatingTreeSanitizer, A.AllowAll, A.ThemeClock, A.ThemeToggle, A.TimeElement0, A.TimeKeeper]);
    _inheritMany(J.Interceptor, [J.JSBool, J.JSNull, J.JavaScriptObject, J.JSNumber, J.JSString]);
    _inheritMany(J.JavaScriptObject, [J.LegacyJavaScriptObject, J.JSArray, A.EventTarget, A._CssStyleDeclaration_JavaScriptObject_CssStyleDeclarationBase, A.DomException, A.DomImplementation, A.DomTokenList, A.Event, A.Location, A._NodeList_JavaScriptObject_ListMixin, A.__NamedNodeMap_JavaScriptObject_ListMixin]);
    ...
    _inheritMany(A.TimeElement0, [A.DateTimeDisplay, A.ClockDisplay]);
  ...
  var type$ = (function rtii() {
    ...
    return {
      ...
      TimeElement: findType("TimeElement0"),
