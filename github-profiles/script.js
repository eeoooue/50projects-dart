...
(function dartProgram() {
  ...
  var A = {JS_CONST: function JS_CONST() {
    ...
    _EventStreamSubscription$(_target, _eventType, onData, _useCapture, $T) {
      ...
      if (t1 != null && true)
        B.HttpRequest_methods.addEventListener$3(_target, _eventType, t1, false);
    ...
    SearchWidget$($parent, element, textInput) {
      var t1 = new A.SearchWidget($parent, element, textInput);
      t1.SearchWidget$3($parent, element, textInput);
      return t1;
    },
    main() {
      var viewer,
        t1 = document,
        main = t1.getElementById("main"),
        form = t1.getElementById("form"),
        search = t1.getElementById("search");
      t1 = type$.Element;
      if (t1._is(main)) {
        viewer = new A.ProfileViewer(main);
        viewer.refresh$0();
        if (t1._is(form) && type$.TextInputElement._is(search)) {
          A.print("created widget");
          A.SearchWidget$(viewer, form, search);
        }
      }
    },
    AllowAll: function AllowAll() {
    },
    SearchWidget: function SearchWidget(t0, t1, t2) {
      this.parent = t0;
      this.element = t1;
      this.textInput = t2;
    },
    SearchWidget_closure: function SearchWidget_closure(t0) {
      this.$this = t0;
    },
    UserStats: function UserStats(t0, t1, t2) {
      this.publicRepos = t0;
      this.followers = t1;
      this.following = t2;
    },
    UserProfile: function UserProfile(t0, t1, t2, t3) {
      var _ = this;
      _.name = t0;
      _.stats = t1;
      _.userName = t2;
    ...
    UserRepo: function UserRepo(t0, t1) {
      this.name = t0;
      this.url = t1;
    },
    ProfileViewer: function ProfileViewer(t0) {
      this.element = t0;
  ...
  A.SearchWidget.prototype = {
    SearchWidget$3($parent, element, textInput) {
      J.addEventListener$2$x(this.element, "submit", new A.SearchWidget_closure(this));
    }
  };
  A.SearchWidget_closure.prototype = {
    call$1($event) {
      var t1, t2, username;
  ...
  A.UserProfile.prototype = {
    createElement$0(_) {
      var $async$goto = 0,
        $async$completer = A._makeAsyncAwaitCompleter(type$.Element),
        $async$returnValue, $async$self = this, t2, t3, card, t1;
      var $async$createElement$0 = A._wrapJsFunctionForAsync(function($async$errorCode, $async$result) {
        ...
        while (true)
          switch ($async$goto) {
            case 0:
              ...
              t1 = J.getInterceptor$x(card);
              t1.get$classes(card).add$1(0, "card");
              t2 = $async$self.name;
              t3 = $async$self.stats;
              t3 = "" + "<div>" + ('<img src="' + $async$self.avatarUrl + '" alt="' + t2 + '" class="avatar">') + '</div><div class="user-info">' + ("<h2>" + t2 + "</h2>") + $async$self.bio + "<ul>" + ("<li>" + t3.followers + " <strong>Followers</strong></li>") + ("<li>" + t3.following + " <strong>Following</strong></li>") + ("<li>" + t3.publicRepos + " <strong>Repos</strong></li>") + '</ul><div id="repos" class="repo-list"></div></div>';
              t1.setInnerHtml$2$validator(card, t3.charCodeAt(0) == 0 ? t3 : t3, new A.AllowAll());
              $async$goto = 3;
              return A._asyncAwait($async$self.deriveRepos$0(), $async$createElement$0);
            case 3:
              // returning from await.
              $async$self.addReposToCard$1(card);
              $async$returnValue = card;
              // goto return
              $async$goto = 1;
              break;
            case 1:
      ...
      return A._asyncStartSync($async$createElement$0, $async$completer);
    },
    deriveRepos$0() {
      var $async$goto = 0,
        $async$completer = A._makeAsyncAwaitCompleter(type$.void),
        $async$self = this, t1, t2, item, t3, repoName, repoUrl, $async$temp1, $async$temp2, $async$temp3;
      var $async$deriveRepos$0 = A._wrapJsFunctionForAsync(function($async$errorCode, $async$result) {
        if ($async$errorCode === 1)
          return A._asyncRethrow($async$result, $async$completer);
        while (true)
          switch ($async$goto) {
            case 0:
              ...
              $async$goto = 3;
              return A._asyncAwait(A.HttpRequest_getString("https://api.github.com/users/" + $async$self.userName + "/repos?sort=created"), $async$deriveRepos$0);
            case 3:
              // returning from await.
              $async$goto = 2;
              return A._asyncAwait($async$temp3.decode$1(0, $async$result), $async$deriveRepos$0);
            case 2:
              // returning from await.
              t1 = $async$temp1.get$iterator$ax($async$temp2._as($async$result)), t2 = $async$self.repos;
            case 4:
              // for condition
              if (!t1.moveNext$0()) {
                // goto after for
              ...
              repoName = A._asStringQ(t3.$index(item, "name"));
              repoUrl = A._asStringQ(t3.$index(item, "html_url"));
              if (repoName != null && repoUrl != null) {
                B.JSArray_methods.add$1(t2, new A.UserRepo(repoName, repoUrl));
                if (t2.length >= 5) {
                  // goto after for
                  $async$goto = 5;
                  break;
      ...
      return A._asyncStartSync($async$deriveRepos$0, $async$completer);
    },
    addReposToCard$1(card) {
      var $async$goto = 0,
        $async$completer = A._makeAsyncAwaitCompleter(type$.dynamic),
        $async$self = this, t2, t3, t4, _i, repo, t5, el, container, t1;
      var $async$addReposToCard$1 = A._wrapJsFunctionForAsync(function($async$errorCode, $async$result) {
        if ($async$errorCode === 1)
          return A._asyncRethrow($async$result, $async$completer);
        while (true)
          switch ($async$goto) {
            case 0:
              // Function start
              container = card.querySelector(".repo-list");
              t1 = container == null;
              if (!t1)
                J.get$children$x(container).clear$0(0);
              for (t2 = $async$self.repos, t3 = t2.length, t4 = type$.AnchorElement, _i = 0; _i < t2.length; t2.length === t3 || (0, A.throwConcurrentModificationError)(t2), ++_i) {
                repo = t2[_i];
                if (!t1) {
                  t5 = J.get$children$x(container);
                  el = document.createElement("a");
                  J.get$classes$x(el).add$1(0, "repo");
                  if (t4._is(el))
                    B.AnchorElement_methods.set$href(el, repo.url);
                  el.innerText = repo.name;
                  t5.add$1(0, el);
                }
              }
              A.print("added repos to card");
              // implicit return
              return A._asyncReturn(null, $async$completer);
          }
      });
      return A._asyncStartSync($async$addReposToCard$1, $async$completer);
    }
  };
  A.UserRepo.prototype = {};
  A.ProfileViewer.prototype = {
    refresh$0() {
      var $async$goto = 0,
        $async$completer = A._makeAsyncAwaitCompleter(type$.dynamic),
        $async$self = this, t3, card, t1, t2;
      var $async$refresh$0 = A._wrapJsFunctionForAsync(function($async$errorCode, $async$result) {
        if ($async$errorCode === 1)
          return A._asyncRethrow($async$result, $async$completer);
        while (true)
          switch ($async$goto) {
            case 0:
              // Function start
              t1 = $async$self.element;
              t2 = J.getInterceptor$x(t1);
              t2.get$children(t1).clear$0(0);
              t3 = $async$self.currentProfile;
              $async$goto = t3 instanceof A.UserProfile ? 2 : 3;
              break;
            case 2:
              // then
              t3 = t3.createElement$0(0);
              $async$goto = 4;
              return A._asyncAwait(t3, $async$refresh$0);
            case 4:
              // returning from await.
              card = $async$result;
              if (card != null)
                t2.get$children(t1).add$1(0, card);
            case 3:
              // join
              // implicit return
              return A._asyncReturn(null, $async$completer);
      ...
      return A._asyncStartSync($async$refresh$0, $async$completer);
    },
    getUser$1(username) {
      var $async$goto = 0,
        $async$completer = A._makeAsyncAwaitCompleter(type$.void),
        $async$self = this, stats, t2, profile, data, t1, $name, $async$temp1;
      var $async$getUser$1 = A._wrapJsFunctionForAsync(function($async$errorCode, $async$result) {
        if ($async$errorCode === 1)
          return A._asyncRethrow($async$result, $async$completer);
        while (true)
          switch ($async$goto) {
            case 0:
              ...
              $async$goto = 3;
              return A._asyncAwait(A.HttpRequest_getString("https://api.github.com/users/" + username), $async$getUser$1);
            case 3:
              // returning from await.
              $async$goto = 2;
              return A._asyncAwait($async$temp1.decode$1(0, $async$result), $async$getUser$1);
            case 2:
              // returning from await.
              data = $async$result;
              t1 = J.getInterceptor$asx(data);
              $name = A._asStringQ(t1.$index(data, "name"));
              if ($name == null)
                $name = A._asStringQ(t1.$index(data, "login"));
              if ($name != null) {
                stats = $async$self.deriveUserStats$1(data);
                t2 = J.JSArray_JSArray$growable(0, type$.UserRepo);
                profile = new A.UserProfile($name, stats, username, t2);
                t2 = A._asStringQ(t1.$index(data, "avatar_url"));
                if (t2 != null)
                  profile.avatarUrl = t2;
                t1 = A._asStringQ(t1.$index(data, "bio"));
                if (t1 != null)
                  profile.bio = t1;
                $async$self.currentProfile = profile;
                $async$self.refresh$0();
              }
              // implicit return
              return A._asyncReturn(null, $async$completer);
          }
      });
      return A._asyncStartSync($async$getUser$1, $async$completer);
    },
    deriveUserStats$1(user) {
      var t1 = J.getInterceptor$asx(user),
        followers = A._asIntQ(t1.$index(user, "followers")),
        following = A._asIntQ(t1.$index(user, "following")),
        publicRepos = A._asIntQ(t1.$index(user, "public_repos"));
      if (A._isInt(followers) && A._isInt(following) && A._isInt(publicRepos))
        return new A.UserStats(publicRepos, followers, following);
      return new A.UserStats(0, 0, 0);
    }
  };
  (function aliases() {
  ...
  var init = {
    ...
    mangledNames: {},
    types: ["~()", "~(~())", "Null(@)", "Null()", "bool(Node)", "bool(NodeValidator)", "bool(String)", "bool(Element,String,String,_Html5NodeValidator)", "@(@)", "@(@,String)", "@(String)", "Null(~())", "~(@)", "Null(@,StackTrace)", "~(int,@)", "~(Object[StackTrace?])", "Null(Object,StackTrace)", "_Future<@>(@)", "~(Object?,Object?)", "String(HttpRequest)", "~(ProgressEvent)", "~(Event)", "String(String)", "~(Node,Node?)", "bool(Set<String>)", "Element(Node)", "Null(Event)"],
    interceptorsByTag: null,
    leafTags: null,
    arrayRti: Symbol("$ti")
  };
  A._Universe_addRules(init.typeUniverse, JSON.parse('{"PlainJavaScriptObject":"LegacyJavaScriptObject","UnknownJavaScriptObject":"LegacyJavaScriptObject","JavaScriptFunction":"LegacyJavaScriptObject","AbortPaymentEvent":"Event","ExtendableEvent":"Event","AElement":"SvgElement","GraphicsElement":"SvgElement","_ResourceProgressEvent":"ProgressEvent","AudioElement":"HtmlElement","MediaElement":"HtmlElement","ShadowRoot":"Node","DocumentFragment":"Node","XmlDocument":"Document","Window":"EventTarget","CDataSection":"CharacterData","Text":"CharacterData","MathMLElement":"Element","HtmlFormControlsCollection":"HtmlCollection","JSBool":{"bool":[],"TrustedGetRuntimeType":[]},"JSNull":{"Null":[],"TrustedGetRuntimeType":[]},"JSArray":{"List":["1"],"Iterable":["1"]},"JSUnmodifiableArray":{"JSArray":["1"],"List":["1"],"Iterable":["1"]},"ArrayIterator":{"Iterator":["1"]},"JSNumber":{"num":[]},"JSInt":{"int":[],"num":[],"TrustedGetRuntimeType":[]},"JSNumNotInt":{"num":[],"TrustedGetRuntimeType":[]},"JSString":{"String":[],"TrustedGetRuntimeType":[]},"LateError":{"Error":[]},"EfficientLengthIterable":{"Iterable":["1"]},"ListIterable":{"Iterable":["1"]},"ListIterator":{"Iterator":["1"]},"MappedIterable":{"Iterable":["2"],"Iterable.E":"2"},"MappedIterator":{"Iterator":["2"]},"MappedListIterable":{"ListIterable":["2"],"Iterable":["2"],"ListIterable.E":"2","Iterable.E":"2"},"WhereIterable":{"Iterable":["1"],"Iterable.E":"1"},"WhereIterator":{"Iterator":["1"]},"NullError":{"TypeError":[],"Error":[]},"JsNoSuchMethodError":{"Error":[]},"UnknownJsTypeError":{"Error":[]},"_StackTrace":{"StackTrace":[]},"Closure":{"Function":[]},"Closure0Args":{"Function":[]},"Closure2Args":{"Function":[]},"TearOffClosure":{"Function":[]},"StaticClosure":{"Function":[]},"BoundClosure":{"Function":[]},"_CyclicInitializationError":{"Error":[]},"RuntimeError":{"Error":[]},"_AssertionError":{"Error":[]},"JsLinkedHashMap":{"MapBase":["1","2"],"Map":["1","2"],"MapBase.K":"1","MapBase.V":"2"},"LinkedHashMapKeyIterable":{"Iterable":["1"],"Iterable.E":"1"},"LinkedHashMapKeyIterator":{"Iterator":["1"]},"_Error":{"Error":[]},"_TypeError":{"TypeError":[],"Error":[]},"_Future":{"Future":["1"]},"AsyncError":{"Error":[]},"_AsyncCompleter":{"_Completer":["1"]},"_Zone":{"Zone":[]},"_RootZone":{"_Zone":[],"Zone":[]},"_LinkedHashSet":{"SetBase":["1"],"Set":["1"],"Iterable":["1"],"SetBase.E":"1"},"_LinkedHashSetIterator":{"Iterator":["1"]},"ListBase":{"List":["1"],"Iterable":["1"]},"MapBase":{"Map":["1","2"]},"SetBase":{"Set":["1"],"Iterable":["1"]},"_SetBase":{"SetBase":["1"],"Set":["1"],"Iterable":["1"]},"_JsonMap":{"MapBase":["String","@"],"Map":["String","@"],"MapBase.K":"String","MapBase.V":"@"},"_JsonMapKeyIterable":{"ListIterable":["String"],"Iterable":["String"],"ListIterable.E":"String","Iterable.E":"String"},"JsonCodec":{"Codec":["Object?","String"]},"int":{"num":[]},"Set":{"Iterable":["1"]},"AssertionError":{"Error":[]},"TypeError":{"Error":[]},"ArgumentError":{"Error":[]},"RangeError":{"Error":[]},"IndexError":{"Error":[]},"UnsupportedError":{"Error":[]},"UnimplementedError":{"Error":[]},"StateError":{"Error":[]},"ConcurrentModificationError":{"Error":[]},"StackOverflowError":{"Error":[]},"_StringStackTrace":{"StackTrace":[]},"Element":{"Node":[],"EventTarget":[]},"HttpRequest":{"EventTarget":[]},"Node":{"EventTarget":[]},"ProgressEvent":{"Event":[]},"_Html5NodeValidator":{"NodeValidator":[]},"HtmlElement":{"Element":[],"Node":[],"EventTarget":[]},"AnchorElement":{"Element":[],"Node":[],"EventTarget":[]},"AreaElement":{"Element":[],"Node":[],"EventTarget":[]},"BaseElement":{"Element":[],"Node":[],"EventTarget":[]},"BodyElement":{"Element":[],"Node":[],"EventTarget":[]},"CharacterData":{"Node":[],"EventTarget":[]},"Document":{"Node":[],"EventTarget":[]},"_ChildrenElementList":{"ListBase":["Element"],"List":["Element"],"Iterable":["Element"],"ListBase.E":"Element"},"FormElement":{"Element":[],"Node":[],"EventTarget":[]},"HtmlCollection":{"ListBase":["Node"],"ImmutableListMixin":["Node"],"List":["Node"],"JavaScriptIndexingBehavior":["Node"],"Iterable":["Node"],"ListBase.E":"Node","ImmutableListMixin.E":"Node"},"HtmlDocument":{"Node":[],"EventTarget":[]},"HttpRequestEventTarget":{"EventTarget":[]},"InputElement":{"TextInputElement":[],"Element":[],"Node":[],"EventTarget":[]},"_ChildNodeListLazy":{"ListBase":["Node"],"List":["Node"],"Iterable":["Node"],"ListBase.E":"Node"},"NodeList":{"ListBase":["Node"],"ImmutableListMixin":["Node"],"List":["Node"],"JavaScriptIndexingBehavior":["Node"],"Iterable":["Node"],"ListBase.E":"Node","ImmutableListMixin.E":"Node"},"SelectElement":{"Element":[],"Node":[],"EventTarget":[]},"TableElement":{"Element":[],"Node":[],"EventTarget":[]},"TableRowElement":{"Element":[],"Node":[],"EventTarget":[]},"TableSectionElement":{"Element":[],"Node":[],"EventTarget":[]},"TemplateElement":{"Element":[],"Node":[],"EventTarget":[]},"_Attr":{"Node":[],"EventTarget":[]},"_NamedNodeMap":{"ListBase":["Node"],"ImmutableListMixin":["Node"],"List":["Node"],"JavaScriptIndexingBehavior":["Node"],"Iterable":["Node"],"ListBase.E":"Node","ImmutableListMixin.E":"Node"},"_AttributeMap":{"MapBase":["String","String"],"Map":["String","String"]},"_ElementAttributeMap":{"MapBase":["String","String"],"Map":["String","String"],"MapBase.K":"String","MapBase.V":"String"},"_ElementCssClassSet":{"SetBase":["String"],"Set":["String"],"Iterable":["String"],"SetBase.E":"String"},"_EventStream":{"Stream":["1"]},"_EventStreamSubscription":{"StreamSubscription":["1"]},"NodeValidatorBuilder":{"NodeValidator":[]},"_SimpleNodeValidator":{"NodeValidator":[]},"_TemplatingNodeValidator":{"NodeValidator":[]},"FixedSizeListIterator":{"Iterator":["1"]},"_SameOriginUriPolicy":{"UriPolicy":[]},"_ValidatingTreeSanitizer":{"NodeTreeSanitizer":[]},"CssClassSetImpl":{"SetBase":["String"],"Set":["String"],"Iterable":["String"]},"FilteredElementList":{"ListBase":["Element"],"List":["Element"],"Iterable":["Element"],"ListBase.E":"Element"},"AttributeClassSet":{"SetBase":["String"],"Set":["String"],"Iterable":["String"],"SetBase.E":"String"},"SvgElement":{"Element":[],"Node":[],"EventTarget":[]},"AllowAll":{"NodeValidator":[]}}'));
  A._Universe_addErasedTypes(init.typeUniverse, JSON.parse('{"EfficientLengthIterable":1,"_SetBase":1,"Converter":2}'));
  var string$ = {
